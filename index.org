...
The way things are drawn doesn't work so well, because shaded walls and lit walls don't draw in the right order (shaded first, lit second). And there are several other things that didn't work well. I think I'll have to come back to this later, after working on enemies. If I still can't make it fun I might go back to tile-based visibility.

At least the game is working again.
